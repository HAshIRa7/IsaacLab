...
def reset_root_state_uniform(
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    positions = root_states[:, :, 0:3] + env.scene.env_origins[env_ids].unsqueeze(dim=1) + rand_samples[:, 0:3].unsqueeze(dim=1)
    orientations_delta = math_utils.quat_from_euler_xyz(rand_samples[:, 3], rand_samples[:, 4], rand_samples[:, 5])
    orientations = math_utils.quat_mul(root_states[:, :, 3:7], orientations_delta.unsqueeze(dim=1).repeat(1, root_states.shape[1], 1))
    # velocities
    range_list = [velocity_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=asset.device)
    rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=asset.device)
